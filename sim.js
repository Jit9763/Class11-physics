...
function activateSim(id) {
  console.log("🚀 activateSim called with id:", id);

  const card = document.getElementById(id);
  const simId = card.getAttribute('data-sim-id');
  const placeholder = card.querySelector('.sim-placeholder');
  console.log("activateSim called with simId:", simId); // 👈 yahan daalo
  ...
  switch(simId) {
    ...
    case "Sheet6":
  console.log("👉 Entered Sheet6 case"); // debug trace

  placeholder.innerHTML = `
    <div style="background: white; padding: 15px; border-radius: 8px; border: 1px solid #ddd;">
      <canvas id="sheet6Canvas" width="${canvasWidth}" height="${canvasHeight}" style="background:#fff;"></canvas>
      <div style="margin-top:10px; text-align:center;">
        <button id="openSheet6Btn" style="padding: 8px 16px; font-size:14px; cursor:pointer;">
          Open xyz3.html
        </button>
      </div>
    </div>`;

  const btn6 = document.getElementById("openSheet6Btn");
  if (btn6) {
    console.log("✅ Button element found for Sheet6");
    btn6.onclick = () => {
      console.log("🎯 Button clicked, opening xyz3.html...");
      window.open("xyz3.html", "_blank");
    };
  } else {
    console.error("❌ Button element NOT found for Sheet6");
  }
  break;
    case "Sheet5": // Image style Distance vs Displacement
    console.log("👉 Entered Sheet5 case with simId:", simId)
      placeholder.innerHTML = `
        <div style="background: white; padding: 15px; border-radius: 8px; border: 1px solid #ddd;">
...
//add more cases for other sheets as needed
   ...
   default:
  console.log("⚠️ No matching case for simId:", simId);
...
function runSheet6Sim() {
  ...
  // Canvas par ek message draw karna
  ctx.font = "20px Arial";
  ctx.fillStyle = "blue";
  ctx.fillText("Simulator for Sheet6 is active.", 50, 150);

  // Button click event
  const btn = document.getElementById("openSheet6Btn");
  if (btn) {
